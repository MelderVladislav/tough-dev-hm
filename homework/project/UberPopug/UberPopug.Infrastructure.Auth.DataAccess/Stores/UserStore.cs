...
namespace UberPopug.Infrastructure.Auth.DataAccess.Stores
{
      ...
      where TUser: User
   {
      ...
      public async Task<AuthServiceResult<TUser>> InsertUser(TUser user, string[] roles)
      {
         ...
         {
            return new AuthServiceResult<TUser>(AuthErrorCode.ExistingIdentity);
         }

         var dbRoles = dbContext.Roles.Where(dbRole => roles.Contains(dbRole.Name)).ToList();

         user.UserRoles = dbRoles.Select(role => new UserRole { Id = Guid.NewGuid(), UserId = user.Id, RoleId = role.Id }).ToList();
