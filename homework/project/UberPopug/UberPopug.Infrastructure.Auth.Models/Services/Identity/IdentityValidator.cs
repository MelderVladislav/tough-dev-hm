...
namespace UberPopug.Infrastructure.Auth.Models.Services.Identity
{
   public class IdentityValidator
   {
      ...
      public AuthServiceResult<bool> Validate(string login, string password, string email)
      {
         ...
         {
            return new AuthServiceResult<bool>(false, AuthErrorCode.LoginIsEmpty);
         }

         if (string.IsNullOrEmpty(password))
         {
            return new AuthServiceResult<bool>(false, AuthErrorCode.PasswordIsEmpty);
         }

         if (string.IsNullOrEmpty(email))
         {
            return new AuthServiceResult<bool>(false, AuthErrorCode.EmailIsEmpty);
         }
         ...
         var hasMinMaxChars = new Regex(@".{" + minPasswordLenght.ToString() + "," + maxPasswordLenght +"}");

         var errorsList = new List<AuthServiceError>();

         if (!hasMinMaxChars.IsMatch(password))
         {
            errorsList.Add(new AuthServiceError(AuthErrorCode.PasswordIsEmpty, $"Password should has minimal {minPasswordLenght} and maximum {maxPasswordLenght} digit lenght"));
         }
         ...
         if (identityConfiguration.IdentityValidationParameters != null)
         {
            ...
            if (identityConfiguration.IdentityValidationParameters.UpperCaseLetterRequired && !hasUpperChar.IsMatch(password))
            {
            ...
            if (identityConfiguration.IdentityValidationParameters.LowerCaseLetterRequired && !hasLowerChar.IsMatch(password))
            {
            ...
            if (identityConfiguration.IdentityValidationParameters.SpecialCharacterRequired && !hasSymbols.IsMatch(password))
            {
               errorsList.Add(new AuthServiceError(AuthErrorCode.SpecialCharRequired));
            }
